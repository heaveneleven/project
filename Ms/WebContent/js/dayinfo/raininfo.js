...
//	$("#hide").val(counts);
	...
	//	$("#list").jqGrid('getGridParam',"day/rain.do?date="+realdate+"&position=1");
		$("#list").jqGrid('setGridParam',
		   {	
			 url : "day/rain.do?date="+realdate
			// position : '1',
			// date : realdate
		   }).trigger("reloadGrid");
	});
	...
	$("#choseDate").datepicker({	
        ...
        //  minDate:'2015-02-05',//最小日期  
          ...
          onSelect: function(selectedDate) {//选择日期后执行的操作  
        	    ...
        	    $("#list").jqGrid('setGridParam',
   				   {	
   					 url : "day/rain.do?date="+$("#choseDate").val()
   				   }).trigger("reloadGrid");
          }  
	});
$("#list").jqGrid({ 
		url:"day/rain.do?date="+getTime(0),
		caption: "日雨情",
		height : 'auto',
		datatype: "json", 
		colNames:['基站编码', '基站名','基站类型','当日雨量','来报次数'],
		colModel:[
		           ...
		           {name:'todaydrp',index:'todaydrp', width:'90',align:'center',
		        	   formatter:function(value,options,row){
			        		    if(value==null)
			        			     return '0.0';
			        		    else
			        		    	 return parseFloat($.trim(value)).toFixed(1);
		           }}, 
		           
		           {name:'counts',index:'counts', width:'77',align:'center',
		        	   formatter:function(value,options,row){
		        		   if(value==null)
		        			   return '0';
		        		   else
	...
	.navButtonAdd('#gridPager',{
	   ...
	   onClickButton: function(){ 
		   ...
		   window.location.href="export/dayrain.do?date="+$("#choseDate").val();
	   }, 
	      position:"last"
	})
